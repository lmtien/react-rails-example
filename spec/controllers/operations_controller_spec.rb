require 'rails_helper'


RSpec.describe OperationsController, type: :controller do

  let(:valid_attributes) {
    skip("Add a hash of attributes valid for your model")
  }

  let(:invalid_attributes) {
    skip("Add a hash of attributes invalid for your model")
  }

  let(:valid_session) { {} }

  describe "GET #index" do
    it "assigns all operations as @operations" do
      operation = Operation.create! valid_attributes
      get :index, params: {}, session: valid_session
      expect(assigns(:operations)).to eq([operation])
    end
  end

  describe "GET #show" do
    it "assigns the requested operation as @operation" do
      operation = Operation.create! valid_attributes
      get :show, params: {id: operation.to_param}, session: valid_session
      expect(assigns(:operation)).to eq(operation)
    end
  end

  describe "GET #new" do
    it "assigns a new operation as @operation" do
      get :new, params: {}, session: valid_session
      expect(assigns(:operation)).to be_a_new(Operation)
    end
  end

  describe "GET #edit" do
    it "assigns the requested operation as @operation" do
      operation = Operation.create! valid_attributes
      get :edit, params: {id: operation.to_param}, session: valid_session
      expect(assigns(:operation)).to eq(operation)
    end
  end

  describe "POST #create" do
    context "with valid params" do
      it "creates a new Operation" do
        expect {
          post :create, params: {operation: valid_attributes}, session: valid_session
        }.to change(Operation, :count).by(1)
      end

      it "assigns a newly created operation as @operation" do
        post :create, params: {operation: valid_attributes}, session: valid_session
        expect(assigns(:operation)).to be_a(Operation)
        expect(assigns(:operation)).to be_persisted
      end

      it "redirects to the created operation" do
        post :create, params: {operation: valid_attributes}, session: valid_session
        expect(response).to redirect_to(Operation.last)
      end
    end

    context "with invalid params" do
      it "assigns a newly created but unsaved operation as @operation" do
        post :create, params: {operation: invalid_attributes}, session: valid_session
        expect(assigns(:operation)).to be_a_new(Operation)
      end

      it "re-renders the 'new' template" do
        post :create, params: {operation: invalid_attributes}, session: valid_session
        expect(response).to render_template("new")
      end
    end
  end

  describe "PUT #update" do
    context "with valid params" do
      let(:new_attributes) {
        skip("Add a hash of attributes valid for your model")
      }

      it "updates the requested operation" do
        operation = Operation.create! valid_attributes
        put :update, params: {id: operation.to_param, operation: new_attributes}, session: valid_session
        operation.reload
        skip("Add assertions for updated state")
      end

      it "assigns the requested operation as @operation" do
        operation = Operation.create! valid_attributes
        put :update, params: {id: operation.to_param, operation: valid_attributes}, session: valid_session
        expect(assigns(:operation)).to eq(operation)
      end

      it "redirects to the operation" do
        operation = Operation.create! valid_attributes
        put :update, params: {id: operation.to_param, operation: valid_attributes}, session: valid_session
        expect(response).to redirect_to(operation)
      end
    end

    context "with invalid params" do
      it "assigns the operation as @operation" do
        operation = Operation.create! valid_attributes
        put :update, params: {id: operation.to_param, operation: invalid_attributes}, session: valid_session
        expect(assigns(:operation)).to eq(operation)
      end

      it "re-renders the 'edit' template" do
        operation = Operation.create! valid_attributes
        put :update, params: {id: operation.to_param, operation: invalid_attributes}, session: valid_session
        expect(response).to render_template("edit")
      end
    end
  end

  describe "DELETE #destroy" do
    it "destroys the requested operation" do
      operation = Operation.create! valid_attributes
      expect {
        delete :destroy, params: {id: operation.to_param}, session: valid_session
      }.to change(Operation, :count).by(-1)
    end

    it "redirects to the operations list" do
      operation = Operation.create! valid_attributes
      delete :destroy, params: {id: operation.to_param}, session: valid_session
      expect(response).to redirect_to(operations_url)
    end
  end

end
